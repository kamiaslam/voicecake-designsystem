...
const Sidebar = ({
    ...
    if (!mounted) {
        return (
            <div className="fixed top-0 left-0 bottom-0 flex flex-col w-85 p-5 bg-b-surface1">
                <div className="animate-pulse">
                    <div className="h-8 bg-b-surface2 rounded mb-5"></div>
                    <div className="space-y-2">
                        {[...Array(8)].map((_, i) => (
                            <div key={i} className="h-12 bg-b-surface2 rounded"></div>
                        ))}
                    </div>
                </div>
            </div>
